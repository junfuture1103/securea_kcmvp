 ...
 * Byte *o: 출력
 */
int Crypt(const Byte *i, int Nr, const Byte *rk, Byte *o) {
...
int ARIA_encrypt(void* context, unsigned char* input, unsigned int inputLength, unsigned char* output, unsigned int* outputLength) {
	...
	// START testing endianness.
	int little_endian = 0;

    b = (Byte*)(&NUMBER);
    if (b[0] == 0x42) {
		// We are on LITTLE_ENDIAN platform.
        little_endian = 1;
    }
    else {
		// We are on BIG_ENDIAN platform.
        little_endian = 0;
    }
...
// 실사용용: 제공된 마스터키/키길이로 ECB 복호
int ARIA_decrypt(const Byte* mk, int keyBits,
    const Byte* in, unsigned int inLen,
    Byte* out)
{
    if (!mk || !in || !out) return -1;
    if (inLen % 16 != 0)    return -2;

    Byte rk[16 * 17];
    int Nr = DecKeySetup(mk, rk, keyBits);
    unsigned int off = 0;
    while (off < inLen) {
        Crypt(in + off, Nr, rk, out + off);
        off += 16;
    }
    return 0;
}